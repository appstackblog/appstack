// aim_smooth_pro.js (ESM)
// Usage: import { createAimController } from "./aim_smooth_pro.js";
...
export function createAimController(options = {}) {
...
/* Example (delta mode)
import { createAimController } from "./aim_smooth_pro.js";

const aim = createAimController({
  enabled: true,
  inputMode: "delta",
  sensitivity: 1.0,
  emaRate: 30,
...
aim.setReticle({x: 640, y: 360});

let last = performance.now();
function frame(mouseDelta){
  const now = performance.now();
  const dt = (now - last) / 1000;
  last = now;

  const { reticle } = aim.update({ dt, mouseDelta });
  return reticle;
}
*/